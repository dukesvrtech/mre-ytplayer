...
export class YoutubeSelectionsController {
		...
		if (!cardsBase) {
			...
			if (streams.length) {
				...
				for (const stream of streams) {
					...
					prev = stream.id
					const card = this.createVideoCard(cardsBase, stream, i);
					cards.push(card);
					if (i % gridWidth === 0) {
						row++;
					}
	...
	setup = () => {
		if (!this.context.ytSelectionsPager) {
			...
			this.context.ytSelectionsPager.totalCount = pageSize;
	...
	displayMovieSelectionPicker = async (soloUser: MyScreenUser, karaokeEnabled = false) => {
		...
		const val =
			await soloUser.prompt(
				`Enter keywords to search for ${karaokeEnabled ? 'Karaoke songs' : "YouTube videos"}.`, true);
		...
		if (this.context.searchTerm && this.context.searchTerm !== this.previousSearch) {
			...
			if (this.context.ytSelectionPanel.appearance.enabled) {
				this.closeMovieSelectionPicker();
			}
			if (!this.context.ytSelectionPanel.appearance.enabled) {
				await openPanel();
			}
	...
	// 	if (this.context.ytSelectionsPager
	// 	&& !this.context.ignoreClicks && this.context.ytSelectionPanel.appearance.enabled) {
	...
	// 	}
	// }
	handleCloseButton = (soloUser: MyScreenUser) => {
		if (this.context.ytSelectionsPager
			&& !this.context.ignoreClicks
			&& this.context.ytSelectionPanel.appearance.enabled) {
			try {
				this.closeMovieSelectionPicker();
			} finally {
				this.context.ignoreClicks = false;
	...
	attachBehaviors = () => {
		if (this.context?.ytSelectionPanel?.appearance?.enabled) {
			...
			//     this.context.ytPagerButtons.nextPageButton
			//     .setBehavior(MRE.ButtonBehavior).onClick(this.handlePageNextButton)
			// }
			// if (this.context?.ytPagerButtons?.prevPageButton) {
			//     this.context.ytPagerButtons.prevPageButton
			//     .setBehavior(MRE.ButtonBehavior).onClick(this.handlePrevPageButton)
			// }
			...
			for (const playButton of
				this.displayCardsBase?.findChildrenByName(playButtonName, true) as MyScreenPlayButton[]) {
		...
		MRE.Actor.Create(this.context, {
			actor: {
				...
				text: {
					...
					contents: wordWrap(videoStream.title?.replace(/[^\x20-\x7E]/g, ""), {
						width: 40,
						indent: "",
						trim: true,
						cut: false
