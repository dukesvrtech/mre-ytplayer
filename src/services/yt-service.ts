...
export const getVideoStreamFromYT = async (id: string): Promise<YouTubeVideoStream> => {
	try {
		...
		if (info?.videoDetails) {
			...
			const videoStream: YouTubeVideoStream = {
			...
			if (!format) {
				...
				if (hls) {
					videoStream.uri = hls?.url;
					videoStream.live = true;
					videoStream.duration = "99:99:99";
				}
			} else {
				videoStream.uri = format.url + '&.mp4';
			}
			shotYTCache.set(cacheKey, videoStream, 600);
			return videoStream;
		}
...
export const searchYoutube = async (q: string): Promise<YoutubeSearchResult> => {
